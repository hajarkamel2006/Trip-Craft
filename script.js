...
function renderItineraryDays() {
    ...
    itineraryState.forEach((day, index) => {
        ...
        col.innerHTML = `
            <div class="glass-card p-3 p-md-4 h-100 d-flex flex-column border" style="min-height: 280px;">
                <div class="d-flex align-items-center justify-content-between pb-3 mb-3 border-bottom border-sand-200">
                    <h3 class="brand-font fs-6 fw-bold text-navy mb-0 text-truncate" title="${day.name}">${day.name}</h3>
                    <span class="badge bg-sand-100 text-muted border border-sand-200 flex-shrink-0 ms-2">${day.items.length} items</span>
                </div>
                <div class="day-drop-zone flex-grow-1 d-flex flex-column gap-2"
                    ...
                    ${day.items.map((item, itemIdx) => `
                        <div class="itinerary-item-card p-2 d-flex align-items-center gap-2 shadow-sm w-100 overflow-hidden">
                            <img src="${item.img}" alt="${item.title}" title="${item.title}" class="rounded-3 object-cover flex-shrink-0" style="width: 40px; height: 40px;" />
                            <div class="flex-grow-1 overflow-hidden" style="min-width: 0;">
                                <h4 class="fw-bold text-navy text-truncate mb-1" style="font-size: 13px; line-height: 1.2;" title="${item.title}">${item.title}</h4>
                                <input type="text" value="${item.time || '10:00 AM'}" 
                                       title="Activity scheduled time"
                                       aria-label="Activity Time"
                                       onchange="updateItemTime(${index}, ${itemIdx}, this.value)" 
                                       class="form-control form-control-sm py-0 px-1 text-muted" 
                                       style="font-size: 11px; width: 80px; height: 22px;" />
                            </div>
                            <button onclick="removeItineraryItem(${index}, ${itemIdx})" 
                                    title="Remove item"
                                    aria-label="Remove item"
                                    class="btn btn-link text-muted p-1 hover-danger flex-shrink-0 text-decoration-none">
...
function selectHotelTier(rate, btn) {
    hotelRate = rate;
    document.querySelectorAll('#view-budget .tier-btn').forEach(b => {
        if (b.innerText.includes('Premium') || b.innerText.includes('5-Star') || b.innerText.includes('Ultra Resort')) {
...
function selectTransTier(rate, btn) {
    transRate = rate;
    document.querySelectorAll('#view-budget .tier-btn').forEach(b => {
        if (b.innerText.includes('Standard') || b.innerText.includes('Private Car') || b.innerText.includes('VIP')) {
            b.classList.remove('active');
        }
    });
    btn.classList.add('active');
...
function quickAddActivity(btn) {
    const card = btn.closest('.activity-source-card');
    if (!card) return;
    ...
    if (itineraryState.length === 0) {
        addNewDay();
    }

    itineraryState[0].items.push(item);
    saveState();
    renderItineraryDays();
